...
- [components/RichTextEditor.vue](file://practice_problems_web/src/components/RichTextEditor.vue)
...
## 更新摘要
**变更内容**
- 在“可复用 UI 组件”章节中新增了“RichTextEditor（富文本编辑器）”子章节，详细描述新组件的功能、配置和与父组件的交互。
- 更新了“PointEditor（知识点编辑器）”子章节，反映其重构后通过组合 `RichTextEditor` 组件实现编辑功能的变更。
- 更新了“详细组件分析”中的架构总览图，以体现 `RichTextEditor` 组件的引入和 `PointEditor` 组件的重构。
- 更新了“依赖关系分析”中的依赖图，以反映新的组件依赖关系。
...
RTE["components/RichTextEditor.vue"]
end
subgraph "类型"
...
- [components/RichTextEditor.vue](file://practice_problems_web/src/components/RichTextEditor.vue#L1-L266)
...
- 视图与组件
  ...
  - components/PointEditor.vue：知识点内容编辑器，重构后集成新的 `RichTextEditor` 组件，提供富文本编辑、图片上传、朗读播放与语音控制功能
  - components/RichTextEditor.vue：新的可复用富文本编辑器组件，基于 `@wangeditor/editor-for-vue` 封装，提供高级粘贴处理和自定义功能。
...
- 组件层：可复用组件（QuestionDrawer、PointEditor、RichTextEditor 等）提升 UI 一致性与复用性
...
- **功能特性**
  - 基于 `@wangeditor/editor-for-vue` 封装，提供现代化的富文本编辑体验。
  - 支持代码块高亮，预设了 Java、JavaScript、Python、Go、HTML、CSS、SQL 等常用语言。
  - **高级粘贴处理**：智能识别粘贴内容，支持直接粘贴图片文件（自动上传）、保留 HTML 格式（表格、列表、缩进）以及保留纯文本的空格和缩进。
  - **图片上传**：集成 `api/point.ts` 中的 `uploadImage` 方法，支持拖拽、粘贴或点击工具栏上传图片，并自动插入到编辑器中。
  - **自定义分割线**：提供 `insertCustomDivider` 方法，可插入带有醒目文字的分割线，增强内容结构。
- **与 API 的交互**
  - `uploadImage(file, pointId)`：用于处理图片上传。
- **与父组件的通信**
  - 通过 `v-model` 双向绑定 `modelValue` 实现内容同步。
  - 通过 `defineExpose` 暴露 `insertCustomDivider` 和 `getEditor` 方法，供父组件调用。
...
#### PointEditor（知识点编辑器）
- **功能特性**
  - **重构后**，该组件不再直接集成富文本编辑器，而是作为容器，通过 `RichTextEditor` 组件来提供核心的富文本编辑功能。
  - 集成 `RichTextEditor` 组件，继承其所有编辑、粘贴和图片上传能力。
  - 上传图片并插入编辑器（调用 `RichTextEditor` 的上传功能）。
  - 文本朗读（SpeechSynthesis），支持音色、语速、暂停/继续/停止。
  - 选择文本朗读与全文朗读。
  - **新增功能**：支持插入自定义分割线，通过调用 `RichTextEditor` 暴露的 `insertCustomDivider` 方法实现。
- **与 API 的交互**
  - updatePoint（保存内容）
  - uploadImage（上传图片，由 `RichTextEditor` 触发）
- **与父组件的通信**
  - 通过 `content` 和 `update` 事件实现内容的双向绑定。
  - 通过 `refresh-bindings` 事件通知父组件刷新绑定列表。
  - 通过 `cache-point` 和 `navigate-to-point` 事件支持知识点间的导航。
...
- 视图依赖
  ...
  - PointEditor 依赖 RichTextEditor
  - Login/index.vue 依赖 request.ts
- API 客户端依赖
  - api/*.ts 依赖 utils/request.ts 与 types/index.ts
- 组件依赖
  - QuestionDrawer 依赖 api/question.ts
  - PointEditor 依赖 api/point.ts 与 RichTextEditor
  - RichTextEditor 依赖 api/point.ts 与 @wangeditor/editor-for-vue
...
- 组件拆分：HeaderSection、DetailPanel、QuestionDrawer、PointEditor、RichTextEditor 等职责清晰，利于按需渲染
...
## 结论
该前端应用采用清晰的分层架构：入口初始化、路由守卫、请求封装、视图/组件与 API 客户端协同工作。通过集中逻辑（useHomeLogic）、可复用组件与统一的 API 客户端，实现了良好的可维护性与扩展性。当前状态管理以组件内状态与简单全局状态为主，满足当前业务规模；未来可根据需要引入更完善的全局状态方案（如 Pinia）以进一步降低组件间耦合。本次更新引入了 `RichTextEditor` 组件，实现了富文本编辑功能的模块化和复用，`PointEditor` 组件通过组合该新组件完成了重构，使代码结构更加清晰，职责更加单一。